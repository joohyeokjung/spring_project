...
public class CharacterController {
	...
	public ResponseEntity<String> post(@RequestBody CharacterVO chvo){
		return chsv.register(chvo) > 0 ? new ResponseEntity<String>("1", HttpStatus.OK)	: new ResponseEntity<String>(HttpStatus.INTERNAL_SERVER_ERROR);
	}
	
	@DeleteMapping(value = "/{mno}", produces = {MediaType.TEXT_PLAIN_VALUE})
	public ResponseEntity<String> erase(@PathVariable("mno") long mno){
		return chsv.removeAll(mno) > 0 ? new ResponseEntity<String>("1", HttpStatus.OK)	: new ResponseEntity<String>(HttpStatus.INTERNAL_SERVER_ERROR);
	}
}
