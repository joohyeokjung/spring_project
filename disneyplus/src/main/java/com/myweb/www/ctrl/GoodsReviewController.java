...
public class GoodsReviewController {
	...
	public ResponseEntity<String> post(@RequestBody GoodsReviewVO grvo) {
		return grsv.register(grvo) > 0 ? new ResponseEntity<String>("1", HttpStatus.OK) : new ResponseEntity<String>(HttpStatus.INTERNAL_SERVER_ERROR);
	}
	...
	public ResponseEntity<String> erase(@PathVariable("grno") long grno) {
		return grsv.remove(grno) > 0 ? new ResponseEntity<String>("1", HttpStatus.OK) :	new ResponseEntity<String>(HttpStatus.INTERNAL_SERVER_ERROR);
	}
}
