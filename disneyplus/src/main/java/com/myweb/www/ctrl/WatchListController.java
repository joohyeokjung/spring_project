...
public class WatchListController {
	...
	public ResponseEntity<String> post(@RequestBody WatchListVO wvo) {
		return wsv.register(wvo) > 0 ? new ResponseEntity<String>("1", HttpStatus.OK) : new ResponseEntity<String>(HttpStatus.INTERNAL_SERVER_ERROR);
	}
	
	@DeleteMapping(value = "/{wno}", produces = {MediaType.TEXT_PLAIN_VALUE})
	public ResponseEntity<String> erase(@PathVariable("wno") long wno) {
		return wsv.remove(wno) > 0 ? new ResponseEntity<String>("1", HttpStatus.OK)	: new ResponseEntity<String>(HttpStatus.INTERNAL_SERVER_ERROR);
	}
