...
<form action="/goods/modify" method="post" enctype="multipart/form-data">
	...
	<span style="position: relative; display: block;" class="mb-3">
		<textarea rows="3" cols="6" class="my-form2" name="description" placeholder="상품 상세 정보">${gdto.gvo.description }</textarea>
	</span>
	...
	<span style="position: relative; display: block;" class="mb-3">
		...
		<button type="button" id="trigger" class="my-btn d-block mb-3">굿즈 이미지</button>
	</span>
	            	...
	            	<c:forEach items="${gfList}" var="gfvo" varStatus="status">
	  					...
	  					<c:choose>
	  						<c:when test="${gfvo.fileType > 0}">
		  						<div>
		  							<img src="/upload/${fn:replace(gfvo.saveDir,'\\','/')}/${gfvo.uuid}_th_${gfvo.fileName}" style="width:100px; height:80px;">
		  						</div>
	  						</c:when>
	  						<c:otherwise>
	  							<div>
	...
	<button class="my-btn my-5 w-100" id="regBtn" type="submit">수정하기</button>
</form>
