...
<nav class="navbar navbar-expand-lg navbar-light bg-light">
	<div class="container-fluid">
		<a class="navbar-brand" href="/"><img class="logo"
			src="/resources/img/disneypluslogo.png"></a>
		...
		<div class="collapse navbar-collapse" id="navbarNavDropdown">
			<ul class="navbar-nav">
				<li class="nav-item"><a class="nav-link" href="/"><i
						class="fas fa-home my-i"></i> <span class="underline">홈</span></a></li>
				<li class="nav-item"><a class="nav-link" href="/rank/list"><i
						class="far fa-chart-bar my-i"></i> <span class="underline">랭킹</span></a>
				</li>
				<li class="nav-item"><a class="nav-link" href="/search/list"><i
						class="fas fa-search my-i"></i> <span class="underline">검색</span></a>
				</li>
				<li class="nav-item"><a class="nav-link" href="/watchlist/list"><i
						class="fas fa-plus my-i"></i> <span class="underline">관심컨텐츠</span></a>
				</li>
				<li class="nav-item"><a class="nav-link"
					href="/movies/list?genre=${genre }"><i class="fas fa-film my-i"></i>
						<span class="underline">영화</span></a></li>
				<li class="nav-item"><a class="nav-link"
					href="/series/list?genre=${genre }"><i class="fas fa-tv my-i"></i>
						<span class="underline">시리즈</span></a></li>
				<li class="nav-item"><a class="nav-link"
					href="/goods/list?company=disney"><i class="fas fa-store my-i"></i>
						<span class="underline">굿즈</span></a></li>

				<li class="nav-item"><sec:authentication
						property="principal.mvo.email" var="authEmail" /> <a
					class="nav-link"
					href="/cart/list?email=${authEmail }&profileName=${profile.profileName}"><i
						class="fas fa-shopping-cart my-i"></i> <span class="underline">구매목록</span></a>
				</li>

				<li class="nav-item dropdown my-dropdown"
					style="position: absolute; right: 3%; min-width: 10rem;"><c:if
						test="${profile.icon != null}">
						<a class="nav-link" style="text-align: right;" href="#"
							id="navbarDropdownMenuLink" role="button"
							data-bs-toggle="dropdown" aria-expanded="false"> <img
							id="icon" style="width: 48px; height: 48px; border-radius: 70%;"
							src="<spring:url value='${profile.icon}'/>">
						</a>
					</c:if>
					<ul class="dropdown-menu" aria-labelledby="navbarDropdownMenuLink">
						<sec:authorize access="isAuthenticated()">
							...
							<li><a class="dropdown-item"
								href="/profile/login?email=${authEmail}">프로필 로그인</a></li>
							<li><a class="dropdown-item" href="/profile/selectIcon">프로필
									추가</a></li>
							<li><a class="dropdown-item"
								href="/profile/profileList?email=${authEmail}">프로필 수정</a></li>
							<li><a class="dropdown-item"
								href="/member/detail?email=${authEmail}">계정</a></li>
							<c:if
...
<sec:authentication property="principal.mvo.authList" var="auths"/>
<input type="hidden" id="getauth" value="${auths.stream().anyMatch(authVO -> authVO.auth.equals('ROLE_ADMIN')).get() }">
<c:set var="mvo" value="${mdto.mvo}" />
<c:set var="mfList" value="${mdto.mfList }" />
<div class="bgImg" style="background-image: url(/upload/${mvo.bgImg});">
	<div class="img-wrapper">
		<div class="main-wrapper">
			...
			<span class="age-wrapper"> <c:choose>
					...
					<c:when test="${mvo.ageLimit eq 19}">
			...
			</span> <span class="age-wrapper"> ${mvo.openingYear} • <c:choose>
			...
			<div class="play-div">
				<a href="/series/play?sno=${season[0].sno}" class="playbtn"><i
                  ...
                  <i class="fas fa-minus fa-2x" id="minusWishList"></i>
					...
					<sec:authorize access="isAuthenticated()"/>
						...
						<c:if
							...
							<div class="d-inline">
								<a class="my-btn"
									href="/series/modify?mno=${mvo.mno}&pageNo=${pgvo.pageNo}&qty=${pgvo.qty}&genre=${pgvo.genre}"
							...
							<div class="d-inline">
								<a class="my-btn" href="/series/season/register?mno=${mvo.mno}"
									style="margin-top: 4px; margin-right: 3px;">시즌 추가</a>
							</div></c:if>
			...
			<nav class="info">
				<div class="d-info" id="season">에피소드</div>
			...
			<div id="seasonV">
				<div>
					<div class="slick">
						<div class="slick-list">
							<div class="slick-track">
								<c:if test="${mvo.season ne null}">
									<c:forEach var="i" begin="1" end="${mvo.season}" step="1">
										<div class="slick-slide">
											<div>
												<button class="seasonbtn" type="button" data-sindex="${i}">시즌
													${i}</button>
											</div>
										</div>
									</c:forEach>
					...
					<div id="details_episodes">
						<div class="my-row row-cols-1 row-cols-md-3 g-4">

							<c:forEach items="${season}" var="svo" varStatus="status">
								<div class="col">
									<div class="my-card">
										<a href="/series/play?sno=${svo.sno}"
											style="background-image: url(/upload/${svo.thumbnail});"
											class="card-img-top my-poster2"></a>
										<div class="card-body">
											<h5 class="card-title">${status.count}.
												${svo.title }&nbsp;
												<sec:authorize access="isAuthenticated()">
												<sec:authentication property="principal.mvo.authList" var="auths"/>
												<c:if
													...
													<a href="/series/season/modify?sno=${svo.sno}"
														class="btn btn-light my-btn-sm">수정</a>
												</c:if>
												</sec:authorize>
												
											</h5>
											<p class="card-text">${svo.description}</p>
										</div>
									</div>
								</div>
					...
					<div class="m-info">
						...
						<div class="m-infoRL">
							...
							<div class="m-info-items">
								...
								<p class="m-info-item">${mvo.openingYear}</p>
							</div>
			...
			<div id="reviewV">
				<h5 class="pb-3">
					리뷰 작성 <span class="my-goods-company"> Review Post</span> <img
						alt="" src="/resources/img/logo/bar.png" width=100% height=1px;>
				</h5>
