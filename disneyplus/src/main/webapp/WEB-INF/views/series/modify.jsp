...
<div class="py-3 px-5 register">
	...
	<form action="/series/modify" method="post" enctype="multipart/form-data">
		...
		<div class="mb-3" style="display: inline-block; margin-right: 30px">
			<span class="margin-r">영화</span> 
			<label class="el-switch">
				<input type="checkbox" name="movie" value="true" <c:if test="${mvo.movie eq true}">checked</c:if>>
				<span class="el-switch-style"></span>
			</label>
		</div>

		<div class="mb-3" style="display: inline-block;">
			<span class="margin-r">시리즈</span> 
			<label class="el-switch"> 
				<input type="checkbox" name="movie" value="false" <c:if test="${mvo.movie eq false}">checked</c:if>> 
				<span class="el-switch-style"></span>
			</label>
		</div>
		...
		<span style="position: relative; display: block;" class="mb-3">
			<textarea rows="3" cols="6" class="my-form2" name="description">${mvo.description}</textarea>
		</span> 
		<span style="position: relative; display: block;" class="mb-3">
			<textarea rows="3" cols="6" class="my-form2" name="description2">${mvo.description2}</textarea>
		</span> 
		<span style="position: relative; display: inline-block;" class="mb-3">
			<select class="my-form" name="company" style="width: 300px">
				...
				<option value="national" <c:if test="${mvo.company eq 'national'}">selected</c:if>>네셔널지오그래픽</option>
		...
		<span style="position: relative; display: block;" class="mb-3">
			<input type="number" class="my-form" name="openingYear" value="${mvo.openingYear}">
		</span> 
		<span style="position: relative; display: block;" class="mb-3">
			<input type="number" class="my-form" name="ageLimit" value="${mvo.ageLimit}">
		</span> 
		<span style="position: relative; display: block;" class="mb-3">
			<input type="number" class="my-form" name="season" value="${mvo.season}">
		</span> 
		<span style="position: relative; display: block;" class="mb-3">
			<input type="text" class="my-form" name="director" value="${mvo.director}">
		</span> 
		<span style="position: relative; display: block;" class="mb-3">
			<input type="text" class="my-form" name="actor" value="${mvo.actor}">
		</span> 
		<span style="position: relative; display: block;" class="mb-3">
			<input type="file" class="my-form" name="files" id="files" style="display: none;" placeholder="포스터">
			<button type="button" id="trigger" class="btn btn-outline-primary btn-block d-block mb-3">포스터 등록</button>
		</span>
		...
		<c:forEach items="${mfList}" var="mfvo">
			...
			<span class="badge bg-secondary rounded-pill">${mfvo.fileSize} Byte</span>
			<button type="button" data-uuid="${mfvo.uuid}" class="file-x">X</button>
		</c:forEach>
		<button class="w-100 btn btn-primary btn-lg my-5" id="regBtn" type="submit">수정하기</button>
	</form>
</div>
