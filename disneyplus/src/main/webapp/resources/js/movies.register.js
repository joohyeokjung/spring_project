...
const regExpImg = new RegExp(".(jpg|jpeg|png|gif|jfif)$");
...
document.addEventListener("change", (e) => {
  if (e.target.id == "files") {
    ...
    for (let file of fileObjects) {
      ...
      ul += `${
        validResult
          ? '<div class="fw-bold">업로드가능'
          : '<div class="fw-bold text-danger">업로드불가'
      }</div>`;
      ul += `${file.name}</div>`;
      ul += `<span class="badge bg-${
        validResult ? "success" : "danger"
      } rounded-pill">${file.size} Bytes</span></li>`;
    }
    ul += "</ul>";
    div.innerHTML = ul;
