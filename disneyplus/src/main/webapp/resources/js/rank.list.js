    ...
    spreadRankListFromServer(pageNo).then(result => {
        ...
        if (result.rankList.length) {
            ...
            for (const mvo of result.rankList) {
                let a = `<a href="/${mvo.movie > 0 ? 'movies' : 'series'}/detail?mno=${mvo.mno}" class="col-md-15 col-sm-3 my-poster" style="background-image: url(/upload/${mvo.posterImg});"><img class="my-rank" src="/resources/img/${rankNum}.png"></a>`
                div.innerHTML += a;
                rankNum += 1;
            }
