...
async function getListofGenre(genre, page) {
  try {
    const resp = await fetch("/series/" + genre + "/" + page);
    ...
    if (result.list.length) {
      ...
      if (result.list.length) {
        for (let mvo of result.list) {
          let a = `<a
           href="/series/detail?mno=${mvo.mno}&pageNo=${result.pgvo.pageNo}&qty=${result.pgvo.qty}&genre=${result.pgvo.genre}"
           class="col-md-15-me col-sm-3 my-posters"
           style="background-image: url(/upload/${mvo.posterImg});">`;
          ul.innerHTML += a;
        }
      }
